...
class XmppInterface
  ...
  def run
    @jabber = Jabber::Simple.new(APP_CONFIG[:xmpp_account], APP_CONFIG[:xmpp_password])

    RAILS_DEFAULT_LOGGER.info("Starting XMPP Interface")
  ...
  def bot_user
    bot ||= User.new(:login => APP_CONFIG[:bot_name], :nick => APP_CONFIG[:bot_name])
  end
  ...
  def add_user(user_id)
    ...
    if(user.save)
      @jabber.add(user_id)    
      send_message(user, "Hello komrade! Welcome to #{APP_CONFIG[:bot_name]} /help for help")
      send_message(user, "History at #{APP_CONFIG[:base_url]}/messages or use the /hist command")
      users = online_users.collect {|u| u.nick || u.login }
